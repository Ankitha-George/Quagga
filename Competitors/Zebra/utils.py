...
class EmailFiles:
    def __init__(self, df, limit=None, skip=0):
        self.df = df
        self.limit = limit
        self.current_file_index = skip
    ...
    def __next__(self):
        if self.limit is not None and (self.limit + self.current_file_index) <= self.run:
            raise StopIteration()

        if self.current_file_index >= len(self.df):
            raise StopIteration()

        row = self.df.iloc[self.current_file_index]
        self.current_file_index += 1
        try:
            with open(row['file'], 'r', errors='ignore') as f:
                self.run += 1
                file = f.read()
                if len(file) < 100:
                    return self.__next__()
                return AnnotatedEmail(row, False)
        except Exception as e:
            print('Error loading file:', row['file'])
            print(e)
            return self.__next__()
...
class AnnotatedEmail(Email):
    def __init__(self, df_row, skip_blank=False, **kwargs):
        self.skip_blank = skip_blank
        self.perturbation = kwargs.get('perturbation', 0.0)
        try:
            self.annotation = json.loads(df_row['ann'])
            super().__init__(df_row['type'], df_row['id'], self.annotation['meta'].get('header', {}))
        except json.decoder.JSONDecodeError:
            print('Error loading JSON Annotation File for:', df_row['filename'])
            raise
...
class AnnotatedEmails:
    def __init__(self, df, feature_function, skip_blank=False, perturbation=0.0):
        self.skip_blank = skip_blank
        ...
        for index, row in df.iterrows():
            ...
            content = row['ann']
            if 'eval' in id:
                self.emails_eval.append(AnnotatedEmail(row, skip_blank, perturbation=self.perturbation))
            elif 'test' in id:
                self.emails_test.append(AnnotatedEmail(row, skip_blank, perturbation=self.perturbation))
            else:
                self.emails_train.append(AnnotatedEmail(row, skip_blank, perturbation=self.perturbation))
    ...
    class AnnotatedEmailsIterator:
        ...
        def _iterator(self, split):
            for _, row in self.df.iterrows():
                if split in row['type']:
                    yield AnnotatedEmail(row, skip_blank=False, perturbation=0.0)
